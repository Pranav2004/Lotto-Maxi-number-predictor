...
def validate_date_range(start_date: datetime, end_date: datetime) -> bool:
    ...
    if (end_date - start_date).days > 365 * 10:
        print("Warning: Date range is very large (>10 years). This may take a while.")
        response = input("Continue? (y/N): ").strip().lower()
        return response in ['y', 'yes']
    
    # Check if end date is in the future
    if end_date > datetime.now():
        print("Warning: End date is in the future. Using today's date instead.")
        return True
...
def run_recommendations_interactive():
    ...
    print("2. Cold Numbers (overdue/infrequent)")
    print("3. Balanced (mixed approach)")
